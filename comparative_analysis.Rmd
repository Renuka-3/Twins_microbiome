...
```{r TvNT, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
library(mia)
library(vegan)
library(scater)
library(dplyr)
...
##working##
#  Comparative analysis between Twins, siblings and unrelated pairs (rarefraction)
...
# Load data
tse <- readRDS("data/twins_tse.rds")
tse_sub <- subsetByPrevalentFeatures(tse,rank = "Phylum", prevalence = 20/ 100)
di <- as.matrix(vegdist(t(assay(tse_sub, "counts")), method="bray"))
di <- reshape2::melt(di) %>% `colnames<-`(c("X","Y", "value"))
#spling location-wise
...
```{r twinsup, message=FALSE, echo=FALSE, warning=FALSE}
library(mia)
library(ggplot2)
library(patchwork)
library(ggsignif)
